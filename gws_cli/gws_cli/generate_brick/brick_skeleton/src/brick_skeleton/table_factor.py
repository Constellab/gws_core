...
@task_decorator(
    "TableFactor", human_name="Table factor", short_description="Apply a factor to a table"
)
class TableFactor(Task):
    ...
    input_specs = InputSpecs(
        {"input_table": InputSpec(Table, human_name="Table", short_description="The input table")}
    )
    output_specs = OutputSpecs(
        {
            "output_table": OutputSpec(
                Table, human_name="Result", short_description="The output table"
            )
        }
    )

    config_specs = ConfigSpecs(
        {"factor": IntParam(human_name="Factor", short_description="Factor to apply to the table")}
    )
