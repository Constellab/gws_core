...
3. **Define input_specs**: An `InputSpecs` object containing a dictionary of InputSpec objects
   - `input_specs = InputSpecs({...})` - wrap the dict with InputSpecs
   - Keys identify inputs in the run method
   - InputSpec parameters:
     - `resource_types`: Resource class or list of classes (first positional argument)
     - `human_name`: Display name
     - `short_description`: Input description
     - `optional`: Allow None or disconnected input

   **Note on Dynamic Inputs**: If the user specifically requests dynamic inputs where they can add/remove ports in the UI, refer to [${GWS_CORE_SRC}/io/dynamic_io.py](${GWS_CORE_SRC}/io/dynamic_io.py) and example at [${GWS_CORE_SRC}/impl/table/smart_tasks/table_smart_multi_transformer.py](${GWS_CORE_SRC}/impl/table/smart_tasks/table_smart_multi_transformer.py). This is rare - for processing collections, use `ResourceSet` or `ResourceList` as a single input instead.

4. **Define output_specs**: An `OutputSpecs` object containing a dictionary of OutputSpec objects
   - `output_specs = OutputSpecs({...})` - wrap the dict with OutputSpecs
   ...
   **Note on Dynamic Outputs**: If the user specifically requests dynamic outputs where they can add/remove ports in the UI, refer to [${GWS_CORE_SRC}/io/dynamic_io.py](${GWS_CORE_SRC}/io/dynamic_io.py) and example at [${GWS_CORE_SRC}/impl/table/smart_tasks/table_smart_multi_transformer.py](${GWS_CORE_SRC}/impl/table/smart_tasks/table_smart_multi_transformer.py). This is rare - for producing collections, return a `ResourceSet` or `ResourceList` as a single output instead.

5. **Define config_specs**: A `ConfigSpecs` object containing a dictionary of Param objects
   - `config_specs = ConfigSpecs({...})` - wrap the dict with ConfigSpecs
...
### Temporary Directory
- `tmp_dir = self.create_tmp_dir()`: Creates auto-deleted temp dir (during `run_after_task`)
- Note: this path can be used for output File and Folder resources because they are moved before cleanup.
...
Here's a complete example of a task file showing all the key components:

```python
from gws_core.config.config_specs import ConfigSpecs, FloatParam, StrParam, ConfigParams, InputSpec, OutputSpec, InputSpecs, OutputSpecs, Task, task_decorator, TaskInputs, TaskOutputs

# Import your resource types
from .my_resource import MyInputResource, MyOutputResource


@task_decorator(
    unique_name="MyTask",
    human_name="My Task",
    short_description="This task processes data and performs operations",
    hide=False,  # Set to True to hide from UI
)
class MyTask(Task):
    """
    [Generated by Task Expert Agent]

    This task performs important operations on input data.

    ## Usage
    - **Input**: MyInputResource containing the data to process
    - **Output**: MyOutputResource with the processed results

    ## Configuration
    - `processing_step`: The step size for processing (default: 0.1)
    - `direction`: The direction of processing (default: "forward")

    ## Notes
    This task demonstrates the complete structure of a Constellab task.
    """

    # Define input specifications - wrap dict with InputSpecs
    input_specs = InputSpecs({
        "input_data": InputSpec(
            MyInputResource,
            human_name="Input Data",
            short_description="The data to process"
        ),
        "optional_data": InputSpec(
            MyInputResource,
            human_name="Optional Data",
            short_description="Additional optional data",
            optional=True
        )
    })

    # Define output specifications - wrap dict with OutputSpecs
    output_specs = OutputSpecs({
        "output_data": OutputSpec(
            MyOutputResource,
            human_name="Output Data",
            short_description="The processed results"
        ),
        "optional_output": OutputSpec(
            MyOutputResource,
            human_name="Optional Output",
            short_description="Additional optional output",
            optional=True
        )
    })

    # Define configuration specifications - wrap dict with ConfigSpecs
    config_specs = ConfigSpecs({
        "processing_step": FloatParam(
            default_value=0.1,
            min_value=0.01,
            max_value=10.0,
            short_description="The step size for processing",
            unit="units"
        ),
        "direction": StrParam(
            default_value="forward",
            allowed_values=["forward", "backward", "left", "right"],
            short_description="The direction of processing"
        )
    })
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        Returns:
            Dictionary of output resources matching output_specs keys
        """
        # Retrieve required inputs
        input_data: MyInputResource = inputs["input_data"]

        # Check for optional inputs
        if inputs.has_resource("optional_data"):
            optional_data: MyInputResource = inputs["optional_data"]
            # Use optional data...

        # Get configuration values
        processing_step = params.get_value("processing_step")
        direction = params.get_value("direction")

        # Log progress
        self.log_info_message(f"Starting processing with step={processing_step}, direction={direction}")
        self.update_progress_value(10, "Initializing...")

        # Perform processing logic
        output_data = MyOutputResource()
        # ... process data ...

        self.update_progress_value(50, "Processing data...")
        # ... more processing ...

        self.update_progress_value(100, "Processing complete")
        self.log_success_message("Task completed successfully")

        # Return outputs matching output_specs keys
        return {
            "output_data": output_data,
            "optional_output": None  # Optional outputs can be None
        }
```
...
Robot tasks examples are available at [${GWS_CORE_SRC}/impl/robot/robot_tasks.py](${GWS_CORE_SRC}/impl/robot/robot_tasks.py).
For dynamic inputs/outputs (rare): [${GWS_CORE_SRC}/io/dynamic_io.py](${GWS_CORE_SRC}/io/dynamic_io.py) and example [${GWS_CORE_SRC}/impl/table/smart_tasks/table_smart_multi_transformer.py](${GWS_CORE_SRC}/impl/table/smart_tasks/table_smart_multi_transformer.py).
...
**Python folder path**:  [${GWS_CORE_SRC}/impl/shell](${GWS_CORE_SRC}/impl/shell)
...
Common resource types available for task inputs and outputs:

- **Table** - 2D excel-like table, pandas DataFrame wrapper with `get_data()` method ([${GWS_CORE_SRC}/impl/table/table.py](${GWS_CORE_SRC}/impl/table/table.py))
- **File** - File resource with path and read/write methods, supports various formats ([${GWS_CORE_SRC}/impl/file/file.py](${GWS_CORE_SRC}/impl/file/file.py))
- **Folder** - Directory resource for managing folder contents and file collections ([${GWS_CORE_SRC}/impl/file/folder.py](${GWS_CORE_SRC}/impl/file/folder.py))
- **Text** - Simple text resource with `get_data()` and `set_data()` methods ([${GWS_CORE_SRC}/impl/text/text.py](${GWS_CORE_SRC}/impl/text/text.py))
- **PlotlyResource** - Interactive plotly figure resource for data visualizations ([${GWS_CORE_SRC}/impl/plotly/plotly_resource.py](${GWS_CORE_SRC}/impl/plotly/plotly_resource.py))
- **ResourceSet** - Named collection of resources (dictionary-like), use for processing/returning multiple resources ([${GWS_CORE_SRC}/resource/resource_set/resource_set.py](${GWS_CORE_SRC}/resource/resource_set/resource_set.py))
- **ResourceList** - Ordered collection of resources (list-like), use for processing/returning multiple resources ([${GWS_CORE_SRC}/resource/resource_set/resource_list.py](${GWS_CORE_SRC}/resource/resource_set/resource_list.py))
