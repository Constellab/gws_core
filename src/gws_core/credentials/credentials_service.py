...
class CredentialsService:
    ...
    def _check_and_build_from_dto(
        ...
        if id_:
            ...
            other = (
                Credentials.select()
                .where(Credentials.id != id_, Credentials.name == save_credentials.name)
                .first()
            )
            if other:
    ...
    @classmethod
    def get_all(cls, page: int = 0, number_of_items_per_page: int = 20) -> Paginator[Credentials]:
    ...
    def get_credentials_data(
        cls, credentials_id: str, user_credentials: UserCredentialsDTO
    ) -> dict:
        ...
        if not Settings.get_instance().is_local_dev_env():
            response: ExternalCheckCredentialResponse = (
                SpaceService.get_instance().check_credentials(user_credentials, False)
            )
