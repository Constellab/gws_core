...
class EntityTag(Model):
    ...
    def delete_by_entity(cls, entity_id: str, entity_type: TagEntityType) -> None:
        cls.delete().where(
            (cls.entity_id == entity_id) & (cls.entity_type == entity_type.value)
        ).execute()
    ...
    @classmethod
    def find_by_tag_and_entity(
        cls, tag: Tag, entity_type: TagEntityType, entity_id: str
    ) -> "EntityTag":
        return (
            cls.select()
            .where(
    ...
    @classmethod
    def find_by_tag_key_and_entity(
        cls, tag_key: str, entity_type: TagEntityType, entity_id: str
    ) -> ModelSelect:
    ...
    def count_by_tag(cls, tag_key: str, tag_value: str) -> int:
        return cls.select().where((cls.tag_key == tag_key) & (cls.tag_value == tag_value)).count()
