...
class TagService():
    ...
    def add_tag_to_entity(cls, tag_entity_type: EntityTagType, entity_id: str,
        ...
        return entity_tags.add_tag_if_not_exist(Tag(tag_key, tag_value),
                                                origin_type=EntityTagOriginType.HUMAN,
                                                origin_id=CurrentUserService.get_and_check_current_user().id)

    @classmethod
    @transaction()
    def save_tags_to_entity(cls, tag_entity_type: EntityTagType, entity_id: str,
                            tags: List[Tag]) -> EntityTagList:

        entity_tags = EntityTagList.find_by_entity(tag_entity_type, entity_id)

        entity_tags.save_tags_to_entity(tags,
                                        origin_type=EntityTagOriginType.HUMAN,
                                        origin_id=CurrentUserService.get_and_check_current_user().id)
    ...
    @classmethod
    def save_tags_dict_to_entity(cls, tag_entity_type: EntityTagType, entity_id: str,
                                 tags_dict: List[TagDict]) -> EntityTagList:

        tags: List[Tag] = TagHelper.tags_dict_to_list(tags_dict)

        return cls.save_tags_to_entity(tag_entity_type, entity_id, tags)
