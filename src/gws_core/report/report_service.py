...
class ReportService():
    ...
    def update_content(cls, report_id: str, report_content: RichTextI) -> Report:
        ...
        cls._refresh_report_associated_views(report)
    ...
    def delete(cls, report_id: str) -> None:
        ...
        EntityTagList.delete_by_entity(EntityTagType.REPORT, report_id)
    ...
    def add_experiment(cls, report_id: str, experiment_id: str,
        ...
        experiment_tags = EntityTagList.find_by_entity(EntityTagType.EXPERIMENT, experiment.id)
        ...
        report_tags.add_tags_to_entity(propagated_tags, update_origin=True)
    ...
    def remove_experiment(cls, report_id: str, experiment_id: str) -> None:
        ...
        rich_text = report.get_content_as_rich_text()
        report_views = rich_text.get_resource_views()
        ...
        experiment_tags = EntityTagList.find_by_entity(EntityTagType.EXPERIMENT, experiment_id)
        propagated_tags = experiment_tags.build_tags_propagated(TagOriginType.EXPERIMENT_PROPAGATED, experiment_id)
        report_tags = EntityTagList.find_by_entity(EntityTagType.REPORT, report_id)
        report_tags.delete_tag_origins(propagated_tags)
    ...
    @classmethod
    def _refresh_report_associated_views(cls, report: Report) -> None:
        ...
        rich_text_views: List[RichTextResourceView] = RichText(report.content).get_resource_views()

        report_tags: EntityTagList = EntityTagList.find_by_entity(EntityTagType.REPORT, report.id)
        ...
        for report_view in report_views:
            if report_view.view.id not in rich_text_view_ids:
                ...
                view_tags = EntityTagList.find_by_entity(EntityTagType.VIEW, report_view.view.id)
        ...
        for rich_text_view in rich_text_views:
            ...
            if rich_text_view.get('view_config_id') not in report_view_ids:
                ...
                if view_config:
                    ...
                    view_tags = EntityTagList.find_by_entity(EntityTagType.VIEW, view_config.id)
    ...
    @classmethod
    def _refresh_report_tags(cls, report: Report) -> None:
        """Recalculate the tags of the report based on the tags of the associated views and experiments and the user tags on the report itself

        :param report: _description_
        :type report: Report
        """

        view_configs: List[ViewConfig] = ReportViewModel.get_by_report(report.id)
        view_tags: List[EntityTag] = EntityTag.find_by_entities(
            EntityTagType.VIEW, [view_config.id for view_config in view_configs])

        experiments: List[Experiment] = cls.get_experiments_by_report(report.id)
        experiment_tags: List[EntityTag] = EntityTag.find_by_entities(
            EntityTagType.EXPERIMENT, [experiment.id for experiment in experiments])

        # report_user_tags: List[EntityTag] = EntityTag.find_by_entity_and_origin_type(
        #     EntityTagType.REPORT, report.id, TagOriginType.USER)

        # concat tag from the most important to the less important
        # all_tags = view_tags + experiment_tags + report_user_tags
        # TODO FIX

        # report_tags: EntityTagList = EntityTagList.find_by_entity(EntityTagType.REPORT, report.id)
        # report_tags.overide_none_user_tags(all_tags)
