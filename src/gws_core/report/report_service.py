...
class ReportService():
    ...
    def add_view_to_content(cls, report_id: str, view_config_id: str) -> Report:
        ...
        return cls.update_content(report_id, rich_text.get_content())
    ...
    def _refresh_report_views_and_tags(cls, report: Report) -> None:
        ...
        It also refresh the associated experiments of the report
        """
        ...
        new_report_views: List[ReportViewModel] = ReportViewModel.get_by_report(report.id)
        associated_experiment = ReportExperiment.find_experiments_by_report(report.id)

        # detect which experiment were added
        for new_view in new_report_views:
            if new_view.view.experiment and new_view.view.experiment not in associated_experiment:
                ReportExperiment.create_obj(new_view.view.experiment, report).save()
