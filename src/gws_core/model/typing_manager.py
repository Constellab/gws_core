...
class TypingManager:
    ...
    def register_typing(cls, typing: Typing, object_class: type[Base]) -> None:
        ...
        if not issubclass(object_class, Base):
            name = object_class.__name__ if object_class.__name__ is not None else str(object_class)
            raise Exception(
    ...
    def _save_object_type_in_db(cls, typing: Typing) -> None:
        try:
            ...
            Logger.error(
                f"Error while saving the typing '{typing.typing_name}', skipping the typing. Error : {err}"
            )
