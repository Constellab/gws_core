...
class TypingSearchBuilder(SearchBuilder):
    ...
    def convert_filter_to_expression(self, filter_: SearchFilterCriteria) -> Expression:
        ...
        if filter_.key == "text":
            ...
            return (
                self._model_type.human_name.contains(filter_.value)
                | self._model_type.short_description.contains(filter_.value)
                | self._model_type.unique_name.contains(filter_.value)
            )
