...
class StreamlitProcess:
    ...
    def __init__(self, port: int, app_type: StreamlitAppType, env_code: str, env_hash: str = None):
        self.port = port
    ...
    def start_streamlit_process(self) -> None:
        ...
        cmd = ['streamlit', 'run', self.get_main_app_file_path(),
               ...
               '--gws_token', self.token,
               '--app_dir', self.get_working_dir(),
               ]
        ...
        str_cmd = ' '.join(cmd)
        self._process = shell_proxy.run_in_new_thread(str_cmd, shell_mode=True)
    ...
    def get_app_full_url(self, app_id: str) -> str:
        ...
        return app.get_app_full_url(self.port, self.token)
    ...
    def get_status_dto(self) -> StreamlitProcessStatusDTO:
        return StreamlitProcessStatusDTO(
            ...
            running_apps=[app.to_dto(self.port, self.token) for app in self.current_running_apps.values()],
