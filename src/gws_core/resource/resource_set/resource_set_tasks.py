...
@task_decorator(
    unique_name="ResourceStacker",
    short_description="Stack a set of resource in a resource set",
    hide=False,
    ...
    config_specs = ConfigSpecs(
        {
            "keys": ParamSet(
                ConfigSpecs(
                    {
                        "key": StrParam(
                            human_name="Resource key",
                            short_description="The key of the resource to stack",
                            optional=True,
                        )
                    }
                ),
        ...
        for resource in resource_list:
            if resource is not None:
                if isinstance(resource, ResourceListBase):
                    ...
                    resource_key = (
                        configs[i]["key"]
                        if len(configs) > i and configs[i] and configs[i]["key"]
                        else resource.name
                    )
...
@task_decorator(
    unique_name="ResourcePicker",
    short_description="Pick a resource from a resource set",
    hide=False,
    style=TypingStyle.material_icon("format_list_bulleted", background_color="#FEC7B4"),
)
                ...
                ConfigSpecs(
                    {
                        "key": StrParam(
                            human_name="Resource key",
                            short_description="The key of the resource to pick",
                        )
                    }
                ),
                optional=False,
