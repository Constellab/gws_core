...
@core_app.get(
    "/resource/{id_}/views/{view_name}/specs",
    tags=["Resource"],
    summary="Get the specs for a view of a resource",
)
def get_view_specs_from_resource(
    id_: str, view_name: str, _=Depends(AuthorizationService.check_user_access_token)
...
def call_view_on_resource(
    ...
    return ResourceService.get_and_call_view_on_resource_model(
        id_, view_name, call_view_params.values, call_view_params.save_view_config
    ).to_dto()


@core_app.post(
    "/resource/{id_}/views/{view_name}/json-file",
    tags=["Resource"],
    summary="Get the json file of a view for a resource",
)
def get_view_json_file(
    id_: str,
    view_name: str,
    call_view_params: CallViewParams,
    _=Depends(AuthorizationService.check_user_access_token),
    ...
    return ResponseHelper.create_file_response_from_object(
        ResourceService.get_view_json_file(
            id_, view_name, call_view_params.values, call_view_params.save_view_config
        ),
        view_name + ".json",
    )
...
def delete_resource(id_: str, _=Depends(AuthorizationService.check_user_access_token)) -> None:
    """
    Delete a resource.
    """
...
@core_app.get(
    "/resource/{id_}/delete/check-impact",
    tags=["Resource"],
    summary="Check impact of resource deletion",
)
def check_impact_delete_resource(
...
@core_app.post(
    "/resource/advanced-search", tags=["Resource"], summary="Advanced search for resource"
)
def advanced_search(
...
@core_app.put(
    "/resource/{id_}/type/{resource_typing_name}", tags=["Files"], summary="Update resource type"
)
def update_file_type(
    id_: str, resource_typing_name: str, _=Depends(AuthorizationService.check_user_access_token)
) -> ResourceModelDTO:
    return ResourceService.update_resource_type(id_, resource_typing_name).to_dto()


@core_app.put(
    "/resource/{id_}/flagged", tags=["Resource"], summary="Update the flagged of a resource"
)
def update_flagged(
...
@core_app.put(
    "/resource/{id_}/folder", tags=["Resource"], summary="Update the folder of a resource"
)
def update_folder(
    id_: str, folder: UpdateFolder, _=Depends(AuthorizationService.check_user_access_token)
) -> ResourceModelDTO:
...
@core_app.post(
    "/resource/{resource_model_id}/import/{importer_typing_name}",
    tags=["Resource"],
...
@core_app.get(
    "/resource/{resource_typing_name}/exporter",
    tags=["Resource"],
...
@core_app.post(
    "/resource/{id_}/export/{exporter_typing_name}", tags=["Resources"], summary="Export a resource"
)
def export_resource(
    id_: str,
...
@core_app.get(
    "/resource-type/{resource_typing_name}/views",
    tags=["Resource type"],
...
@core_app.get(
    "/resource-type/{resource_type}/views/{view_name}/specs",
    tags=["Resource type"],
    summary="Get the specs for a view of a resource type",
)
def get_view_specs_from_type(
    resource_type: str, view_name: str, _=Depends(AuthorizationService.check_user_access_token)
) -> ResourceViewMetadatalDTO:
...
@core_app.get(
    "/resource/import-from-link/config-specs",
    tags=["Share"],
...
@core_app.post(
    "/resource/{resource_id}/export-to-lab", tags=["Share"], summary="Export a resource to a lab"
)
def export_resource_to_lab(
    resource_id: str,
...
@core_app.get(
    "/resource/export-to-lab/config-specs",
    tags=["Share"],
...
@core_app.post(
    "/resource/{resource_id}/share-with-space",
    tags=["Share"],
    summary="Share a resource with the space",
)
def share_resource_with_space(
    resource_id: str,
    request_dto: ShareResourceWithSpaceRequestDTO,
    _=Depends(AuthorizationService.check_user_access_token),
) -> ShareLinkDTO:
