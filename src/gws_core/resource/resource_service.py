...
from ..core.classes.search_builder import SearchFilterCriteria, SearchParams
...
class ResourceService:
    ...
    def check_if_resource_is_used(cls, resource_model: ResourceModel) -> None:
        ...
        if next_scenario:
            raise BadRequestException(
                GWSException.RESOURCE_USED_ERROR.value,
    ...
    def update_folder(cls, resource_id: str, folder_id: str | None) -> ResourceModel:
        ...
        folder: SpaceFolder | None = (
            SpaceFolder.get_by_id_and_check(folder_id) if folder_id else None
        )
    ...
    def get_scenarios_resources(cls, scenario_ids: list[str]) -> list[ResourceModel]:
        ...
        """
        generated_resources = cls.get_scenario_generated_resources(scenario_ids)
    ...
    def get_view_specs_from_resource(
        cls, resource_model_id: str, view_name: str
    ...
    @classmethod
    def get_and_call_view_on_resource_model(
        cls,
    ...
    @classmethod
    def call_view_on_resource_model(
        cls,
        ...
        return CallViewResult(
            view_dto, resource_model.id, view_config, cast(str, view_title), view.get_type(), style
        )
    ...
    def call_view_from_view_config(cls, view_config_id: str) -> CallViewResult:
        ...
        view_runner: ViewRunner = ViewRunner(
            resource, view_config.view_name, view_config.get_config_values()
        )
        ...
        return CallViewResult(
            view_dto,
            view_config.resource_model.id,
            view_config,
            view_config.title,
            view.get_type(),
            view_runner.get_metadata_style(),
        )
    ...
    def search(
        ...
        criteria: SearchFilterCriteria | None = search.get_filter_criteria("include_children_resource")

        # if the criteria is not provided or False, we don't include the children
        if criteria is None or not criteria.value:
            search_builder.add_expression(cast(Field, ResourceModel.parent_resource_id).is_null())
        ...
        include_non_output: SearchFilterCriteria | None = search.get_filter_criteria(
            "include_not_flagged"
        )
    ...
    def search_apps(
        ...
        pagination = search_builder.search_page(page, number_of_items_per_page)
        return pagination
    ...
    def check_column_tags(
        ...
        table = table_resource_model.get_resource(resource_type=Table)
