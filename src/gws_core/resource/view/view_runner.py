...
class ViewRunner:
    ...
    def generate_view(self) -> View:
        ...
        if view is None or not isinstance(view, View):
            raise Exception(
                f"The view method '{view_metadata.method_name}' didn't returned a View object"
            )
    ...
    def _get_and_check_view_meta(self) -> ResourceViewMetaData:
        if self._view_metadata is None:
            self._view_metadata = ViewHelper.get_and_check_view_meta(
                type(self.resource), self.view_name
            )
    ...
    def get_config(self) -> Config:
        config = Config()
        config.set_specs(
            self._get_and_check_view_meta().get_view_specs_from_type(skip_private=False)
        )
