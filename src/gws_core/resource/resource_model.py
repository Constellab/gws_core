...
class ResourceModel(ModelWithUser, ModelWithFolder, NavigableEntity):
    ...
    resource_typing_name = TypedCharField(null=False)

    # Path to the kv store if the kv exists for this resource model
    kv_store_path = NullableCharField(null=True)

    origin = TypedEnumField(choices=ResourceOrigin, default=ResourceOrigin.GENERATED)

    scenario = NullableDeferredForeignKeyField["Scenario"]("Scenario", null=True, index=True)
    folder = NullableForeignKeyField(SpaceFolder, null=True, index=True)
    task_model = NullableDeferredForeignKeyField["TaskModel"]("TaskModel", null=True, index=True)
    ...
    parent_resource_id = NullableForeignKeyIdField("self", null=True, index=True)

    name = TypedCharField(null=False)
    ...
    generated_by_port_name = NullableCharField(null=True)
    ...
    content_is_deleted = TypedBooleanField(default=False)
