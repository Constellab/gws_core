...
from gws_core.resource.resource_dto import (
    ResourceModelDTO,
    ResourceModelExportDTO,
    ResourceOrigin,
    ResourceSimpleDTO,
)
...
class ResourceModel(ModelWithUser, ModelWithFolder, NavigableEntity):
    ...
    kv_store_path: NullableCharField = NullableCharField()

    origin: TypedEnumField[ResourceOrigin] = TypedEnumField(choices=ResourceOrigin, default=ResourceOrigin.GENERATED)

    scenario = NullableDeferredForeignKeyField["Scenario"]("Scenario", index=True)
    folder: NullableForeignKeyField[SpaceFolder] = NullableForeignKeyField(SpaceFolder, index=True)
    task_model = NullableDeferredForeignKeyField["TaskModel"]("TaskModel", index=True)
    ...
    parent_resource_id: NullableForeignKeyIdField = NullableForeignKeyIdField("self", index=True)

    name: TypedCharField = TypedCharField()

    fs_node_model: NullableForeignKeyField[FSNodeModel] = NullableForeignKeyField(FSNodeModel, index=True, backref="+")

    # true when the resource is UPLOADED or is a scenario output
    flagged: TypedBooleanField = TypedBooleanField(default=False)

    # provided when the resource is generated by a scenario, this contains the port name of the task that generated it
    generated_by_port_name: NullableCharField = NullableCharField()
    ...
    content_is_deleted: TypedBooleanField = TypedBooleanField(default=False)
    ...
    def to_export_dto(self) -> ResourceModelExportDTO:
        ...
        return ResourceModelExportDTO(
            **dto.to_json_dict(),
            ...
            generated_by_port_name=self.generated_by_port_name,
            task_model_id=self.task_model.id if self.task_model else None,
        )
