...
class Form(ModelWithUser, NavigableEntity):
    ...
    template_version: TypedForeignKeyField[FormTemplateVersion] = TypedForeignKeyField(
        FormTemplateVersion, backref="forms"
    )

    # Auto-generated by Peewee from the template version FK. Annotated here so type
    # checkers see the raw-id accessor (avoids loading the related row).
    template_version_id: str

    status: TypedEnumField[FormStatus] = TypedEnumField(
        choices=FormStatus, default=FormStatus.DRAFT, index=True
    )

    submitted_at: NullableDateTimeUTC = NullableDateTimeUTC()
    submitted_by: NullableForeignKeyField[User] = NullableForeignKeyField(User, backref="+")

    # Field values keyed by ConfigSpecs key; ParamSet items carry __item_id.
    values: NullableJSONField = NullableJSONField()
