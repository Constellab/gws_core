...
from gws_core.config.config_change_dto import ConfigChangeEntry
...
class FormSaveEvent(Model):
    ...
    form: TypedForeignKeyField[Form] = TypedForeignKeyField(Form, on_delete="CASCADE", backref="save_events")
    user: TypedForeignKeyField[User] = TypedForeignKeyField(User, backref="+")

    # List of FormChangeEntry dicts.
    changes: TypedJSONField = TypedJSONField()

    # Auto-generated by Peewee from the form FK. Annotated here so type
    ...
    def set_changes(self, entries: Sequence[ConfigChangeEntry]) -> None:
