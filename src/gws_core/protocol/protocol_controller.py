...
@core_app.post("/protocol/{id_}/add-template/{scenario_component_id}", tags=["Protocol"],
               summary="Add a viewer link a process' output")
def add_scenario_component_to_protocol(id_: str,
                                       scenario_component_id: str,
                                       _=Depends(AuthService.check_user_access_token)) -> ProtocolUpdateDTO:
    with update_lock:
        return ProtocolService.add_scenario_component_to_protocol(
            protocol_id=id_, scenario_component_id=scenario_component_id).to_dto()
...
class CreateScenarioComponent(BaseModelDTO):
    name: str = None
...
def create_template(id_: str,
                    template: CreateScenarioComponent,
                    _=Depends(AuthService.check_user_access_token)) -> ScenarioComponentDTO:
    return ProtocolService.create_scenario_component_from_id(id_, template.name, template.description).to_dto()
...
def download_template(id_: str,
    ...
    template = ProtocolService.generate_scenario_component(id_)
