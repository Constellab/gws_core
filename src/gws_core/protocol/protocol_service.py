...
class ProtocolService:
    ...
    def add_process_to_protocol_id(
        cls,
        protocol_id: str,
    ...
    @classmethod
    @GwsCoreDbManager.transaction()
    def duplicate_process_to_protocol_id(
    ...
    def add_empty_protocol_to_protocol_id(cls, protocol_id: str) -> ProtocolUpdate:
        ...
        return cls.add_empty_protocol_to_protocol(
            protocol_model=protocol_model, name="Sub-scenario"
        )
    ...
    def add_empty_protocol_to_protocol(
        ...
        return cls.add_process_model_to_protocol(
            protocol_model=protocol_model,
            process_model=child_protocol_model,
            instance_name=instance_name,
        )
    ...
    def add_process_to_protocol(
        ...
        return cls.add_process_model_to_protocol(
            protocol_model=protocol_model, process_model=process_model, instance_name=instance_name
        )
    ...
    def add_process_connected_to_input(
        ...
        for output_spec_name, output_spec in (
            new_process_type.get_output_specs().get_specs().items()
        ):
        ...
        protocol_update_2 = cls.add_connector_to_protocol(
            ...
            protocol_update.process.instance_name,
            output_name,
            existing_process.instance_name,
            input_port_name,
        )
    ...
    @classmethod
    def delete_process_of_protocol_id(
        cls, protocol_id: str, process_instance_name: str
    ) -> ProtocolUpdate:
    ...
    @classmethod
    @GwsCoreDbManager.transaction()
    def delete_process_of_protocol(
        cls, protocol_model: ProtocolModel, process_instance_name: str
    ) -> ProtocolUpdate:
    ...
    def reset_process_of_protocol(
        ...
        return ProtocolUpdate(
            protocol=protocol_model, protocol_updated=True, sub_protocols=sub_resetted_protocols
        )
    ...
    @classmethod
    def _get_process_of_protocol(
        cls, protocol_model_id: str, process_instance_name: str
    ) -> ProcessModel:
    ...
    ########################## CONNECTORS #####################
    @classmethod
    def add_connector_to_protocol_id(
        cls,
        protocol_id: str,
        ...
        return cls.add_connector_to_protocol(
            protocol_model, from_process_name, from_port_name, to_process_name, to_port_name
        )
    ...
    def add_connector_to_protocol(
        ...
        protocol_model.check_is_updatable()
        connector = protocol_model.add_connector(
            from_process_name, from_port_name, to_process_name, to_port_name
        )
    ...
    def delete_connector_of_protocol(
        cls, protocol_id: str, dest_process_name: str, dest_process_port_name: str
    ) -> ProtocolUpdate:
        protocol_model: ProtocolModel = ProtocolModel.get_by_id_and_check(protocol_id)
    ...
    def add_interface_to_protocol_id(
        cls, protocol_model_id: str, target_process_name: str, target_port_name: str
    ) -> ProtocolUpdate:
        protocol_model = cls.get_by_id_and_check(protocol_model_id)
        return cls.add_interface_to_protocol(
            protocol_model,
            protocol_model.generate_interface_name(),
            target_process_name,
            target_port_name,
        )
    ...
    def add_interface_to_protocol(
        ...
        ioface = protocol_model.add_interface(name, target_process_name, target_port_name)
        protocol_model.save_graph()
        return cls._on_protocol_object_updated(
            protocol_model=protocol_model, ioface=ioface, protocol_updated=True
        )

    @classmethod
    def add_outerface_to_protocol_id(
        cls, protocol_id: str, source_process_name: str, source_port_name: str
    ) -> ProtocolUpdate:
        protocol_model = cls.get_by_id_and_check(protocol_id)
        return cls.add_outerface_to_protocol(
            protocol_model,
            protocol_model.generate_outerface_name(),
            source_process_name,
            source_port_name,
        )
    ...
    @classmethod
    def delete_interface_of_protocol_id(
        cls, protocol_id: str, interface_name: str
    ) -> ProtocolUpdate:
        protocol_model = cls.get_by_id_and_check(protocol_id)
        return cls.delete_interface_of_protocol(protocol_model, interface_name)

    @classmethod
    def delete_interface_of_protocol(
        cls, protocol_model: ProtocolModel, interface_name: str
    ) -> ProtocolUpdate:
        protocol_model.check_is_updatable()
    ...
    @classmethod
    def delete_outerface_of_protocol_id(
        cls, protocol_id: str, outerface_name: str
    ) -> ProtocolUpdate:
        protocol_model = cls.get_by_id_and_check(protocol_id)
    ...
    def configure_process(
        cls, protocol_id: str, process_instance_name: str, config_values: ConfigParamsDict
    ) -> ProtocolUpdate:
        protocol_model: ProtocolModel = ProtocolModel.get_by_id_and_check(protocol_id)
    ...
    def add_input_resource_to_protocol_id(
        cls, protocol_id: str, resource_id: str
    ) -> ProtocolUpdate:
        ...
        return cls.add_process_model_to_protocol(protocol_model, source)

    @classmethod
    def add_input_resource_to_process_input(
        cls, protocol_id: str, resource_id: str, process_name: str, input_port_name: str
    ) -> ProtocolUpdate:
    ...
    def _check_scenario_circular_reference(cls, new_resource_id: str, scenario_id: str) -> None:
        ...
        all_previous_exp = resource_nav.get_previous_entities_recursive(
            requested_entities=[NavigableEntityType.SCENARIO]
        )
    ...
    def add_output_task_to_process_ouput(
        cls, protocol_id: str, process_name: str, output_port_name: str
    ) -> ProtocolUpdate:
        """Add a output task to the protocol. And add connector from process to output"""

        return cls.add_process_connected_to_output(
            protocol_id, OutputTask.get_typing_name(), process_name, output_port_name
        )
    ...
    def add_viewer_to_process_output(
        ...
        return cls.add_process_connected_to_output(
            protocol_id, Viewer.get_typing_name(), process_name, output_port_name, viewer_config
        )

    @classmethod
    @GwsCoreDbManager.transaction()
    def add_scenario_template_to_protocol(
        cls, protocol_id: str, scenario_template_id: str
    ) -> ProtocolUpdate:
    ...
    def save_process_layout(
        cls, protocol_id: str, process_instance_name: str, layout: ProcessLayoutDTO
    ) -> None:
        protocol_model: ProtocolModel = ProtocolModel.get_by_id_and_check(protocol_id)
    ...
    def save_interface_layout(
        cls, protocol_id: str, interface_name: str, layout: ProcessLayoutDTO
    ) -> None:
        protocol_model: ProtocolModel = ProtocolModel.get_by_id_and_check(protocol_id)
    ...
    def save_outerface_layout(
        cls, protocol_id: str, outerface_name: str, layout: ProcessLayoutDTO
    ) -> None:
        protocol_model: ProtocolModel = ProtocolModel.get_by_id_and_check(protocol_id)
    ...
    def _add_dynamic_port_to_process(
        ...
        process_model.save()
        new_update = ProtocolUpdate(
            protocol=protocol_model, protocol_updated=False, process=process_model
        )
        return new_update

    @classmethod
    def delete_dynamic_input_port_of_process(
        cls, protocol_id: str, process_name: str, port_name: str
    ) -> ProtocolUpdate:
    ...
    @classmethod
    @GwsCoreDbManager.transaction()
    def _delete_dynamic_port_of_process(
        cls,
        protocol_id: str,
        process_name: str,
        port_name: str,
        ...
        process_model.save()
        new_update = ProtocolUpdate(
            protocol=protocol_model, protocol_updated=True, process=process_model
        )
        return new_update
    ...
    def update_dynamic_input_port_of_process(
        ...
        return cls._update_dynamic_port_of_process(
            protocol_id, process_name, port_name, io_spec, "input"
        )
    ...
    def update_dynamic_output_port_of_process(
        ...
        return cls._update_dynamic_port_of_process(
            protocol_id, process_name, port_name, io_spec, "output"
        )
    ...
    @classmethod
    def rename_process(
        cls, protocol_id: str, process_instance_name: str, custom_name: str
    ) -> ProcessModel:
        protocol_model: ProtocolModel = ProtocolModel.get_by_id_and_check(protocol_id)
    ...
    def update_process_style(
        cls, protocol_id, process_instance_name: str, style: TypingStyle
    ) -> ProcessModel:
        protocol_model: ProtocolModel = ProtocolModel.get_by_id_and_check(protocol_id)
        process_model: ProcessModel = protocol_model.get_process(process_instance_name)
    ...
    def create_scenario_template_from_id(
        ...
        return ScenarioTemplateService.create_from_protocol(
            protocol=protocol_model, name=name, description=description
        )
    ...
    def generate_scenario_template(cls, protocol_id: str) -> ScenarioTemplate:
        ...
        if not protocol_model.scenario:
            raise BadRequestException("Cannot download a protocol without scenario")

        return ScenarioTemplateFactory.from_protocol_model(
            protocol_model, protocol_model.scenario.title, protocol_model.scenario.description
        )
    ...
    def create_community_agent(
        cls, process_id: str, form_data: CommunityCreateAgentDTO
    ) -> CommunityAgentVersionCreateResDTO:
    ...
    def fork_community_agent(
        cls, process_id: str, form_data: CommunityCreateAgentDTO, agent_version_id: str
    ) -> CommunityAgentVersionCreateResDTO:
        version_file: CommunityAgentFileDTO = AgentFactory.generate_agent_file_from_agent_id(
            process_id
        )
    ...
    def create_community_agent_version(
        cls, process_id: str, agent_id: str
    ) -> CommunityAgentVersionCreateResDTO:
        version_file: CommunityAgentFileDTO = AgentFactory.generate_agent_file_from_agent_id(
            process_id
        )
    ...
    ########################## DYNAMIC PARAM #####################
    @classmethod
    def _get_and_check_dynamic_param(
