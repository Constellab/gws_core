...
class ProtocolModel(ProcessModel):
    ...
    def _run_protocol(self) -> None:
            ...
            for process in self.processes.values():
                if process.instance_name not in runned_processes and self.process_is_ready(process):
                    ...
                    self.progress_bar.update_progress(
                        round(len(runned_processes) / count_processes * 100),
        ...
        if len(runned_processes) < count_processes:
            self.progress_bar.add_warning_message("Some processes are not ready to be runned")

    def _run_process(self, process: ProcessModel) -> None:
        self.progress_bar.add_info_message(f"Run process {process.get_name()}")
        process.run()
    ...
    def get_current_progress(self) -> ScenarioProgressDTO:
        ...
        """
        if self.is_finished:
            return ScenarioProgressDTO(
                progress=100, last_message=self.progress_bar.get_last_message()
            )
        if self.is_draft:
    ...
    def get_process_by_instance_path(self, instance_path: str) -> ProcessModel:
        ...
        if not isinstance(process, ProtocolModel):
            raise BadRequestException(
                f"Process '{process.get_instance_name_context()}' is not a protocol"
            )
    ...
    def get_direct_next_processes(self, process_name: str) -> set[ProcessModel]:
        ...
        self._check_instance_name(process_name)
        return {
            connector.right_process
            for connector in self.connectors
            if connector.left_process.instance_name == process_name
        }
    ...
    def get_all_next_processes(
        ...
        :rtype: List[Process]
        """
        return self._get_all_next_processes_recur(
            process_name=process_name,
            root_process_name=process_name,
            skip_processes=set(),
            check_parent_protocol=check_parent_protocol,
    ...
    def _get_all_next_processes_recur(
        ...
        for process in next_processes:
            ...
            all_next_processes.update(
                self._get_all_next_processes_recur(
    ...
    def _check_port(
        self, process_name: str, port_name: str, port_type: Literal["IN", "OUT"]
    ) -> None:
    ...
    def delete_connectors_from_left(
    ...
    def _add_interface(self, name: str, process_name: str, port_name: str) -> IOface:
        ...
        self._interfaces[name] = IOface(
            name=name, process_instance_name=process_name, port_name=port_name
        )
    ...
    def port_is_interface(self, process_instance_name: str, port_name) -> bool:
        ...
        for interface in self.interfaces.values():
            if (
                interface.process_instance_name == process_instance_name
                and interface.port_name == port_name
            ):
                return True
    ...
    def to_protocol_config_dto(
        self, ignore_input_task_config: bool = False
    ) -> ProtocolGraphConfigDTO:
        return ProtocolGraphConfigDTO(
            nodes={
                key: process.to_config_dto(ignore_input_task_config)
    ...
    def to_protocol_dto(self) -> ProtocolDTO:
        ...
        return ProtocolDTO(**process_dto.to_json_dict(), data=self.to_protocol_full_dto())
    ...
    def mark_as_started(self):
        ...
        if not self.progress_bar.is_started:
            ...
            self.progress_bar.add_message(f"Start of protocol '{self.get_instance_name_context()}'")
        else:
    ...
    def _mark_as_partially_run(self):
        if self.is_partially_run:
            return
        self.progress_bar.add_message("The protocol was modified, marking it as PARTIALLY RUN")
            ...
            if process.is_input_task():
                ...
                for connector in connectors:
                    ...
                    new_interfaces[interface_name] = IOFaceDTO(
                        name=interface_name,
                        process_instance_name=connector.right_process.instance_name,
