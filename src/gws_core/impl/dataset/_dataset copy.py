...
class Dataset(Table):
    ...
    def select_by_row_positions(self, indexes: List[int], only_features=False, only_targets=False) -> 'Dataset':
    ...
    def select_by_column_positions(self, indexes: List[int], only_features=False, only_targets=False) -> 'Dataset':
        if not isinstance(indexes, list):
            raise BadRequestException("The indexes must be a list of integers")
        data = DataFrame()
        targets = DataFrame()
    ...
    @staticmethod
    def _feature_name_formater(x):
        return "F"+str(x)

    @staticmethod
    def _target_name_formater(x):
        return "T"+str(x)
    ...
    def _set_features_and_targets(
        ...
        if features is not None:
            ...
            features.index = Dataset._format_names(features.index, mapper=Dataset._row_formater)
            features.columns = Dataset._format_names(features.columns, mapper=Dataset._feature_name_formater)
            self._data = features
        ...
        if targets is not None:
            ...
            targets.index = Dataset._format_names(targets.index, mapper=Dataset._row_formater)
            targets.columns = Dataset._target_name_formater(targets.columns, mapper=Dataset._target_name_formater)
            self._targets = targets
