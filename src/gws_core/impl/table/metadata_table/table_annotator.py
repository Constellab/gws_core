...
@task_decorator(
    unique_name="TableRowAnnotator",
    human_name="Table row annotator",
...
class TableRowAnnotator(Task):
    ...
    input_specs: InputSpecs = InputSpecs(
        {
            "sample_table": InputSpec(
                Table, human_name="Sample table", short_description="Table to annotate"
            ),
            "metadata_table": InputSpec(
                Table,
                human_name="Metadata table",
                short_description="Table containing the metadata",
            ),
        }
    )
    ...
    config_specs = ConfigSpecs(
        {
            "reference_column": StrParam(
                default_value="",
                human_name="Reference column in sample table",
                short_description="Column in the sample table whose values are used for annotation. If empty, it uses the first column.",
            ),
            "metadata_ref_column": StrParam(
                default_value="",
                human_name="Reference column in metadata table",
            ...
            "use_table_row_names_as_ref": BoolParam(
                default_value=False,
                human_name="Use sample table row names as reference",
            ...
            "use_metadata_row_names_as_ref": BoolParam(
                default_value=False,
                human_name="Use metadata table row names as reference",
                visibility="protected",
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        if use_table_row_names_as_ref:
            ...
            self.log_info_message(
                f"Using sample table column '{reference_column}' as reference for annotation"
            )
        else:
        ...
        if use_metadata_row_names_as_ref:
            ...
            self.log_info_message(
                f"Using metadata table column '{metadata_ref_column}' as reference for annotation"
            )
        else:
...
@task_decorator(
    unique_name="TableColumnAnnotator",
    human_name="Table column annotator",
...
class TableColumnAnnotator(Task):
    ...
    * if an `id` matches against a reference value of the `Sample table`, the corresponding column of the `Sample table` is taggeg with the metadata given by the `id`.
    ...
    config_specs = ConfigSpecs(
        {
            "reference_row": StrParam(
                default_value="",
                human_name="Reference row in sample table",
            ...
            "use_table_column_names_as_ref": BoolParam(
                default_value=False,
                human_name="Use sample table column names as reference",
                visibility="protected",
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        if use_table_column_names_as_ref:
            ...
            self.log_info_message(
                f"Using sample table row '{reference_row}' as reference for annotation"
            )
        else:
