...
class TableView(BaseTableView):
    ...
    _specs = ConfigSpecs(
        {
            ...
            "number_of_rows_per_page": IntParam(
                default_value=TabularView.MAX_NUMBER_OF_ROWS_PER_PAGE,
                max_value=TabularView.MAX_NUMBER_OF_ROWS_PER_PAGE,
                min_value=1,
            ...
            "from_column": IntParam(
                default_value=1, human_name="From column", visibility=StrParam.PROTECTED_VISIBILITY
            ),
            "number_of_columns_per_page": IntParam(
                default_value=TabularView.DEFAULT_NUMBER_OF_COLUMNS_PER_PAGE,
                max_value=TabularView.MAX_NUMBER_OF_COLUMNS_PER_PAGE,
                min_value=1,
    ...
    def data_to_dict(self, params: ConfigParams) -> dict:
        tabular_view = TabularView(
            self._table,
            from_row=params["from_row"] - 1,  # - 1 because communication uses 1-based indexing
            from_column=params["from_column"]
            - 1,  # - 1 because communication uses 1-based indexing
