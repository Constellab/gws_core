...
from ...impl.robot.robot_resource import MegaRobot, Robot, RobotAddOn, RobotFood
...
@task_decorator(
    "RobotCreate",
    human_name="Create robot",
    short_description="This task creates a robot",
    hide=True,
)
...
class RobotMove(Task):
    input_specs = InputSpecs(
        {"robot": InputSpec(Robot, human_name="Robot", short_description="The robot to feed")}
    )  # just for testing
    output_specs = OutputSpecs(
        {"robot": OutputSpec(Robot), "food": OutputSpec(RobotFood, optional=True)}
    )
    config_specs = ConfigSpecs(
        {
...
class RobotEat(Task):
    input_specs = InputSpecs(
        {"robot": InputSpec(Robot), "food": InputSpec(RobotFood, optional=True)}
    )
    output_specs = OutputSpecs({"robot": OutputSpec(Robot)})
...
@task_decorator(
    unique_name="RobotAddOnCreate",
    human_name="The travel of `Astro`",
...
@task_decorator(
    unique_name="RobotSugarCreate",
    human_name="Create a sugar type of food",
