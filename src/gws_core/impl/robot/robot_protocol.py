...
from .robot_tasks import RobotAdd, RobotAddOnCreate, RobotCreate, RobotEat, RobotFly, RobotMove
...
class RobotSimpleTravel(Protocol):
    ...
    def configure_protocol(self) -> None:
        ...
        move_1: ProcessSpec = (
            self.add_process(RobotMove, "move_1")
            .set_param("moving_step", 2000)
            .set_param("direction", "west")
        )
...
@protocol_decorator(
    ...
    human_name="The super travel of Astro",
    hide=True,
    style=TypingStyle.material_icon("smart_toy", background_color="#2b6d57"),
)
...
@protocol_decorator(
    ...
    human_name="The world trip of Astro",
    hide=True,
    style=TypingStyle.material_icon("smart_toy", background_color="#2b6d57"),
)
class RobotWorldTravelProto(Protocol):
    ...
    def configure_protocol(self) -> None:
        ...
        facto: ProcessSpec = self.add_process(RobotCreate, "facto")
        fly_1: ProcessSpec = (
            self.add_process(RobotFly, "fly_1")
            .set_param("moving_step", 2000)
            .set_param("direction", "west")
        )
