...
@task_decorator(
    unique_name="ResourceUploaderS3",
    human_name="Upload resource to S3",
...
class ResourceUploaderS3(Task):
    ...
    config_specs = ConfigSpecs(
        {
            ...
            "send_me_an_email": BoolParam(
                human_name="Send me an email when the task is finished", default_value=False
            ),
        }
    )
    ...
    def send_email(self, bucket_name: str, file_name: str) -> None:
        ...
        MailService.send_mail_to_current_user(
            f"""
<p>Hello,</p>

<p>Your resource has been successfully uploaded to S3 bucket '{bucket_name}'.</p>

<p>It was zipped into the following file: <strong>{file_name}</strong></p>""",
            "Resource uploaded to S3",
        )
