...
@s3_server_app.put("/v1/{bucket}/{key}")
...
@s3_server_app.get("/v1/{bucket}/{key}")
def download_object(bucket: str, key: str) -> FileResponse:
...
def list_objects(bucket: str,
                 ...
                 _=Depends(S3ServerAuth.check_s3_server_auth)):
    result = S3ServerService.list_objects(bucket, max_keys=max_keys)

    return ResponseHelper.create_xml_response_from_json({'ListBucketResult': result})


@s3_server_app.head("/v1/{bucket}/{key}")
def head_object(bucket: str, key: str,
                _=Depends(S3ServerAuth.check_s3_server_auth)):
    return {"data": []}


@s3_server_app.delete("/v1/{bucket}/{key}")
