...
from gws_core.impl.s3.s3_server_exception import S3ServerException, S3ServerNoSuchKey
...
class DataHubS3ServerService(AbstractS3Service):
    ...
    def _create_object(self, key: str, data: ByteString, tags: Dict[str, str] = None) -> None:
        with S3ServerContext(self.bucket_name, key):
            ...
            resource_tags.add_tag(
                Tag(
                    self.STORAGE_TAG_NAME,
                    self.STORAGE_TAG_VALUE,
                    is_propagable=False,
                    origins=origins,
                    auto_parse=True,
                )
            ...
            resource_tags.add_tag(
                Tag(self.KEY_TAG_NAME, key, is_propagable=False, origins=origins, auto_parse=True)
            )
    ...
    def delete_object(self, key: str) -> None:
        ...
        with S3ServerContext(self.bucket_name, key):
            EntityNavigatorService.delete_resource(
    ...
    def delete_objects(self, keys: List[str]) -> None:
        ...
        for key in keys:
            ...
            if resource is None:
                Logger.warning(
                    f"Object {key} not found in bucket {self.bucket_name}, skipping deletion."
                )
                continue
    ...
    def _resource_to_s3_object(self, resource: ResourceModel) -> ObjectTypeDef:
        entity_tag: EntityTag = EntityTag.find_by_tag_key_and_entity(
            "key", TagEntityType.RESOURCE, resource.id
        ).first()
        if not entity_tag:
            raise S3ServerException(
                status_code=500,
    ...
    def _get_s3_expression_builder(
        ...
        search_builder.add_tag_filter(
            Tag(self.STORAGE_TAG_NAME, self.STORAGE_TAG_VALUE, auto_parse=True)
        )
        ...
        if prefix:
            if not prefix.endswith("/"):
                prefix += "/"
            search_builder.add_tag_filter(
                Tag(self.KEY_TAG_NAME, prefix, auto_parse=True), SearchOperator.START_WITH
            )
    ...
    def get_object_tags(self, key: str) -> S3GetTagResponse:
        ...
        return {"VersionId": "1", "TagSet": {"Tag": s3_tags}, "ResponseMetadata": None}
    ...
    def update_object_tags_dict(self, key: str, tags: Dict[str, str]) -> None:
        ...
        for tag_key, tag_value in additional_new_tags.items():
            entity_tags.add_tag(
                Tag(tag_key, tag_value, origins=self.get_tag_origin(), auto_parse=True)
            )
