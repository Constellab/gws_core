...
@core_app.post(
    "/share-link/public", tags=["Share"], summary="Generate a public share link for an entity"
)
def generate_public_share_link(
    share_dto: GenerateShareLinkDTO, _=Depends(AuthorizationService.check_user_access_token)
) -> ShareLinkDTO:
...
def delete_share_link(id_: str, _=Depends(AuthorizationService.check_user_access_token)) -> None:
...
@core_app.get(
    "/share-link/{entity_type}/{entity_id}/{link_type}", tags=["Share"], summary="Get share entity"
)
def get_share_entity(
...
def clean_share_links(
    clean_links_dto: CleanLinksDTO, _=Depends(AuthorizationService.check_user_access_token)
) -> None:
    ShareLinkService.clean_links(
        clean_links_dto.clean_expired_links, clean_links_dto.clean_invalid_links
    )
