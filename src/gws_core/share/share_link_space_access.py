...
class ShareLinkSpaceAccessService:
    ...
    @classmethod
    def generate_share_link_space_access(cls, share_link_id: str, user_id: str) -> ShareLinkSpaceAccess:
    ...
    @classmethod
    def find_by_token_and_check_validity(cls, user_access_token: str, share_link_id: str) -> ShareLinkSpaceAccess:
        """Find a share link space access by its token and check if it is valid
        """

        access = next((access for access in cls.accesses if access.user_access_token == user_access_token), None)
    ...
    def _add_access(cls, access: ShareLinkSpaceAccess) -> None:
        ...
        cls.accesses = [access for access in cls.accesses if access.user_id !=
                        access.user_id and access.share_link_id != access.share_link_id]
