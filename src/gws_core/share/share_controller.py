...
@core_app.post(
    "/share/{entity_type}/mark-as-shared/{token}",
    tags=["Share"],
...
@core_app.get(
    "/share/{entity_type}/{entity_id}/shared-to",
    tags=["Share"],
...
@core_app.get(
    "/share/resource/{token}/download", tags=["Share"], summary="Download a zipped entity"
)
...
@core_app.get("/share/scenario/{token}", tags=["Share"], summary="Get info of the shared scenario")
...
@core_app.post(
    "/share/scenario/{token}/resource/{resource_id}/zip",
    tags=["Share"],
...
# Open route to download a resource
@core_app.get(
    "/share/scenario/{token}/resource/{resource_id}/download",
    tags=["Share"],
    summary="Download the resource of a shared scenario",
)
...
###################################### VIEW ######################################
@core_app.post(
    "/share/resource/views/{view_name}",
    tags=["Resource"],
...
def call_view_on_resource(
    ...
    auth_context: AuthContextShareLink = Depends(AuthorizationService.check_share_link),
) -> CallViewResultDTO:
    return ShareService.call_resource_view(
        auth_context.get_share_link(), view_name, call_view_params
    ).to_dto()
