...
def create_empty(
    data: CreateNoteTemplateDTO, _=Depends(AuthorizationService.check_user_access_token)
) -> NoteTemplateDTO:
    return NoteTemplateService.create_empty(data.title).to_dto()


@core_app.post(
    "/note-template/from-note", tags=["Note template"], summary="Create a note template from a note"
)
def create_from_note(
    data: CreateNoteTemplateFromNoteDTO, _=Depends(AuthorizationService.check_user_access_token)
) -> NoteTemplateDTO:
    return NoteTemplateService.create_from_note(data.note_id).to_dto()


@core_app.put(
    "/note-template/{doc_id}/title",
    tags=["Note template"],
...
@core_app.delete(
    "/note-template/{doc_id}", tags=["Note template"], summary="Delete a notetemplate "
)
def delete(doc_id: str, _=Depends(AuthorizationService.check_user_access_token)) -> None:
...
def get_by_id(id_: str, _=Depends(AuthorizationService.check_user_access_token)) -> NoteTemplateDTO:
    return NoteTemplateService.get_by_id_and_check(id_).to_dto()


@core_app.get(
    "/note-template/{id_}/content",
    tags=["Note template"],
    summary="Get a note",
    response_model=None,
)
...
@core_app.post(
    "/note-template/search", tags=["Note template"], summary="Advanced search for note templates"
)
def search(
...
@core_app.get(
    "/note-template/search-name/{name}",
    tags=["Note template"],
