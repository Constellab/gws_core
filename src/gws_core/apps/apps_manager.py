...
class AppsManager:
    ...
    def get_logs_of_app(
        ...
        if not isinstance(app_process, StreamlitProcess | ReflexProcess):
        ...
        # Retrieve the log generated by the app: filter by context_id only (no context)
        # so both the app child process records (STREAMLIT/REFLEX context) and the
        # server-side records about this app (MAIN context stamped via AppLogContext:
        # build, cache clear, nginx registration, plugin install) are returned.
        return LogService.get_logs_between_dates(
            start_date, end_date, context=None, context_id=app_id
        )
