...
class ReflexProcess(AppProcess):
    ...
    def _start_process(self, app: AppInstance) -> SysProc:
        ...
        shell_proxy.working_dir = app.app_dir

        # Must use the shell_mode=False to retrieve the correct pid to check the number of connections
        return shell_proxy.run_in_new_thread(cmd, shell_mode=False, env={
            'GWS_TOKEN': self._token,
            'GWS_APP_ID': app.app_id,
            'GWS_APP_CONFIG_FILE_PATH': app.get_config_file_path(),
        })
