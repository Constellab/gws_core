...
class StreamlitPlugin(AppPluginDownloader):
    ...
    def reset_streamlit_index_html_file(self) -> None:
        ...
        head = streamlit_index.head
        if head is None:
            raise Exception("Streamlit index.html file has no head section")
        self._remove_existing_content(head, self.START_COMMENT, self.END_COMMENT)
        # Remove existing version comment from head
        self._remove_existing_content(head, self.VERSION_COMMENT, self.VERSION_COMMENT)

        # Remove existing auto-added content from body
        body = streamlit_index.body
        if body is None:
            raise Exception("Streamlit index.html file has no body section")
        self._remove_existing_content(body, self.START_COMMENT, self.END_COMMENT)

        # Write the modified HTML back to the destination file
        self._write_streamlit_html_file(streamlit_index)

    def _remove_existing_content(self, container: Tag, start_comment: str, end_comment: str):
        start_comments = container.find_all(
            string=lambda text: isinstance(text, Comment) and start_comment in text
        )
        for start_com in start_comments:
            current = start_com
            elements_to_remove = []
