...
class StreamlitEnvAgent(Task):
    ...
    **Warning**: It is recommended to use code snippets comming from trusted sources.
    ...
    def run_agent(
        ...
        # prepend initialization code to the user code
        init_code = """
from gws_streamlit_env_main import StreamlitEnvMainState

# Initialize GWS - MUST be at the top
StreamlitEnvMainState.initialize()

source_paths = StreamlitEnvMainState.get_source_paths()
params = StreamlitEnvMainState.get_params()
"""
        full_code = init_code + code

        # build the streamlit resource with the code and the resources
        streamlit_resource = StreamlitResource()
        streamlit_resource.set_streamlit_code(full_code)
    ...
    @classmethod
    def build_config_params_dict(cls, code: str, params: dict[str, Any], env: str | None) -> ConfigParamsDict:
