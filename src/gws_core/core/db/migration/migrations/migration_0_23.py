...
class Migration0230(BrickMigration):
    ...
    def _migrate_share_link_columns(cls, sql_migrator: SqlMigrator) -> None:
        ...
        if not ShareLink.table_exists():
            Logger.info("Migration 0.23.0: Table 'gws_share_link' does not exist, skipping")
            return

        share_link_columns = [c for c in ["entity_id", "token"] if ShareLink.column_exists(c)]
        if not share_link_columns:
            Logger.info(
                "Migration 0.23.0: No entity_id/token columns on 'gws_share_link', skipping"
            )
            return

        Logger.info(
            "Migration 0.23.0: Deleting unusable gws_share_link rows with NULL entity_id or token"
        )
        ...
        Logger.info("Migration 0.23.0: Setting gws_share_link.entity_id/token to NOT NULL")
    ...
    def _migrate_protocol_layout(cls, sql_migrator: SqlMigrator) -> None:
        ...
        if not ProtocolModel.table_exists() or not ProtocolModel.column_exists("layout"):
            Logger.info("Migration 0.23.0: No gws_protocol.layout column, skipping")
            return

        empty_layout = ProtocolLayout().serialize()
        Logger.info("Migration 0.23.0: Backfilling NULL gws_protocol.layout columns")
        ProtocolModel.execute_sql(
            f"UPDATE [TABLE_NAME] SET layout = '{empty_layout}' WHERE layout IS NULL"
        )

        Logger.info("Migration 0.23.0: Setting gws_protocol.layout to NOT NULL")
        sql_migrator.alter_column_type(
    ...
    def _migrate_user_columns(cls, sql_migrator: SqlMigrator) -> None:
        ...
        for model in _MODELS_WITH_USER:
            ...
            if not model.table_exists():
                Logger.info(f"Migration 0.23.0: Table '{table_name}' does not exist, skipping")
                continue
            ...
            if not existing_columns:
                Logger.info(
                    f"Migration 0.23.0: No created_by/last_modified_by columns on '{table_name}', skipping"
                )
                continue

            Logger.info(
                f"Migration 0.23.0: Backfilling NULL user columns on '{table_name}'"
            )
            ...
            Logger.info(
                f"Migration 0.23.0: Setting user columns to NOT NULL on '{table_name}'"
            )
    ...
    def _migrate_credentials_type(cls) -> None:
        ...
        Logger.info(
            "Migration 0.23.0: Migrating credentials type to namespaced registry id"
        )
    ...
    def _migrate_process_model_columns(
        ...
        if model.column_exists("scenario_id"):
            ...
            if orphan_count:
                raise Exception(
                    f"Migration 0.23.0: cannot set scenario_id to NOT NULL on '{table_name}': "
        ...
        if model is TaskModel and model.column_exists("parent_protocol_id"):
            ...
            if orphan_count:
                raise Exception(
                    f"Migration 0.23.0: cannot set parent_protocol_id to NOT NULL on '{table_name}': "
    ...
    def _backfill_process_columns(cls, model: type[ProcessModel]) -> None:
        ...
        Logger.info(
            f"Migration 0.23.0: Backfilling NULL columns on '{model.get_table_name()}'"
        )
    ...
    def _alter_process_columns(
        ...
        Logger.info(
            "Migration 0.23.0: Setting instance_name/name/data/scenario_id/progress_bar"
    ...
    def _backfill_progress_bars(cls, model: type[ProcessModel]) -> None:
        ...
        if not orphans:
            Logger.info(f"Migration 0.23.0: No NULL progress_bar_id on '{table_name}'")
            return

        Logger.info(
            f"Migration 0.23.0: Backfilling {len(orphans)} NULL progress_bar_id on '{table_name}'"
        )
