...
class GWSException(Enum):
    ...
    WRONG_CREDENTIALS_USER_NOT_FOUND = (
        "Not authorized. Cannot generate user access token. User not found"
    )
    WRONG_CREDENTIALS_USER_NOT_ACTIVATED = (
        "Not authorized. Cannot generate user access token. User not active"
    )
    INVALID_TOKEN = "Not authorized. Invalid token"
    INVALID_APP_TOKEN = "Not authorized. Invalid app token"
    FUNCTIONALITY_UNAVAILBLE_IN_PROD = (
        "This functionnality is not available in the production environment"
    )
    ...
    RESOURCE_USED_ERROR = (
        "This resource is used in the scenario '{{scenario}}', it can't be modified nor deleted"
    )
    ...
    NOTE_VALIDATION_EXP_OTHER_FOLDER = (
        "The linked scenario '{{title}}' is associated to another folder"
    )
    ...
    NOTE_ADD_EXP_OTHER_FOLDER = (
        "Can't link the scenario to the note because they are in different root folders"
    )
    ...
    NOTE_NO_LINKED_SCENARIO = (
        "The note is not linked with a scenario. Please link it to a scenario to access views."
    )
    DELETE_FOLDER_WITH_SCENARIOS = (
        "The folder can't be deleted because it contains validated scenarios"
    )
    ...
    PROCESS_UPDATE_FISHINED_ERROR = (
        "The process is finished. Please reset the process before modifying it."
    )
