...
class SettingsLoader:
    ...
    def load_brick(self, brick_name: str, parent_name: str = None) -> None:
        ...
        if not os.path.exists(brick_path):
            brick_module.error = (
                f"Folder '{brick_path}' for brick '{brick_name}' is not found. Skipping brick."
            )
        ...
        try:
            ...
            brick_module.error = (
                f"Error: cannot parse the the settings file of brick '{brick_name}'. Error {err}"
            )
            self._save_brick(brick_module)
            return
        ...
        if not brick_settings.version:
            brick_module.error = (
                f"Missing version in settings.json for brick {brick_name}. Skipping brick."
            )
            self._save_brick(brick_module)
            return
        ...
        if brick_settings.environment and brick_settings.environment.pip:
            ...
            for pip_source in brick_settings.environment.pip:
                source_dict = {"source": pip_source.source, "packages": []}
