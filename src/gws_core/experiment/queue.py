...
def _queue_tick(tick_interval, verbose, daemon):
    ...
    try:
        ...
        thread = threading.Timer(tick_interval, _queue_tick, [
...
class Queue(Model):
    ...
    def _tick(cls, verbose=False):
        ...
        if verbose:
            Logger.info(f"Experiment {e.uri}, is_running = {e.is_running}")
        ...
        if verbose:
            Logger.info(f"Start experiment {e.uri}, user={job.user.uri}")
