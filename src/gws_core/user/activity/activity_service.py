...
class ActivityService:
    ...
    def add_with_catch(
        ...
        try:
            ...
            Logger.error(
                f"Error while adding activity {activity_type.value} {object_type.value}. Error : {err}"
            )
            return None
    ...
    def add_or_update_async(
        ...
        thread = Thread(
            target=cls.add_or_update, args=(activity_type, object_type, object_id, user)
        )
        thread.start()
