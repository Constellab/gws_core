...
class NoteService:
    ...
    def _update_note_folder(cls, note: Note, new_folder_id: str | None) -> Note:
        ...
        if new_folder_id:
            ...
            if note.last_sync_at is not None and new_folder != note.folder:
                note_folder = cast(SpaceFolder, note.folder)
                if note_folder.get_root().id != new_folder.get_root().id:
                ...
                SpaceService.get_instance().update_lab_note_folder(
                    note_folder.id, note.id, new_folder.id
                )
    ...
    def update_content(cls, note_id: str, note_content: RichTextDTO) -> Note:
        ...
        if not Settings.get_instance().is_test and not Settings.get_instance().is_local_env():
            note.modifications = SpaceService.get_instance().get_rich_text_modifications(
                cast(RichTextDTO, note.content), note_content, note.modifications
            )
    ...
    @classmethod
    def _check_scenarios_are_in_note_folder(cls, note: Note, scenarios: list[Scenario]) -> None:
        """Check that every scenario linked to the note is in the same folder as the note.

        :param note: note being validated
        :type note: Note
        :param scenarios: scenarios linked to the note
        :type scenarios: list[Scenario]
        :raises BadRequestException: if a scenario belongs to another folder
        """
        note_folder = cast(SpaceFolder, note.folder)
        for scenario in scenarios:
            if scenario.folder and scenario.folder.id != note_folder.id:
                raise BadRequestException(
    ...
    @classmethod
    def _check_resource_views_come_from_note_scenarios(
        cls, rich_text: RichText, scenarios: list[Scenario]
    ) -> None:
        """Check that every resource view of the note content comes from a resource that was
        generated (or used) by one of the scenarios linked to the note.

        :param rich_text: content of the note
        :type rich_text: RichText
        :param scenarios: scenarios linked to the note
        :type scenarios: list[Scenario]
        :raises BadRequestException: if a view references a resource of another scenario
        """
        ...
        for resource_view in resource_views:
            ...
            if not resource.is_manually_generated():
                resource_scenario = cast(Scenario, resource.scenario)
                if resource_scenario.id not in scenario_ids:
                    ...
                    raise BadRequestException(
                        ...
                        {"view_name": view_name, "exp_title": resource_scenario.title},
                    )
            elif not TaskInputModel.resource_is_used_by_scenario(resource.id, scenario_ids):
                raise BadRequestException(
    ...
    @classmethod
    def _validate_note_scenarios(cls, note: Note, scenarios: list[Scenario]) -> None:
        """Validate the scenarios linked to the note that are not validated yet,
        moving them to the note folder.
    ...
    def _validate(cls, note_id: str, folder_id: str | None = None) -> Note:
        ...
        cls._check_scenarios_are_in_note_folder(note, scenarios)

        # check that all resource views are from resource that
        # were generated by scenarios that are linked to the note
        cls._check_resource_views_come_from_note_scenarios(rich_text, scenarios)

        # validate scenario that were not validated
        cls._validate_note_scenarios(note, scenarios)
    ...
    def _synchronize_with_space(cls, note: Note) -> Note:
        ...
        for form_block in rich_text.get_blocks_by_type(RichTextBlockTypeStandard.FORM):
            form_data_block: RichTextBlockForm = form_block.get_data(RichTextBlockForm)
            try:
    ...
    def search(
        ...
        search_builder: SearchBuilder[Note] = NoteSearchBuilder()
    ...
    def get_undo_content(cls, note_id: str, modification_id: str) -> RichTextDTO:
        ...
        return SpaceService.get_instance().get_rich_text_undo_content(
            cast(RichTextDTO, note.content),
            cast(RichTextModificationsDTO, note.modifications),
            modification_id,
        )
    ...
    def rollback_content(cls, note_id: str, modification_id: str) -> Note:
        ...
        note_modifications = cast(RichTextModificationsDTO, note.modifications)
        target_modification = None
        for modif in note_modifications.modifications:
        ...
        modification_index = note_modifications.modifications.index(target_modification)

        modif_type = note_modifications.modifications[modification_index].type
        ...
        note_modifications.modifications = note_modifications.modifications[:modification_index]
