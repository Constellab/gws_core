...
@task_decorator(
    unique_name="InputTaskFromProcessOutput",
...
class InputTaskFromProcessOutput(Task):
    ...
    config_specs = ConfigSpecs(
        {
            "process_model_id": StrParam(
                human_name="Process model id",
                short_description="The id of the process model that contains the output",
            ),
            "process_output_port_name": StrParam(
                human_name="Output port name",
                short_description="The name of the output port of the process model",
            ),
        }
    )
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        if not process_model.is_success:
            if process_model.is_error:
                raise Exception(f"Process '{process_model.name}' is in error")
            else:
                raise Exception(
