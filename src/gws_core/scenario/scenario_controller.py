...
@core_app.post(
    "/scenario/advanced-search", tags=["Scenario"], summary="Advanced search for scenario"
)
def advanced_search(
...
def count_by_title(
    title: str, _=Depends(AuthorizationService.check_user_access_token)
) -> ScenarioCountByTitleResultDTO:
    return ScenarioCountByTitleResultDTO(count=ScenarioService.count_by_title(title))


@core_app.get(
    "/scenario/search-title/{title}", tags=["Scenario"], summary="Search scenario by title"
)
def search_by_title(
    title: str,
    page: int | None = 1,
...
@core_app.get(
    "/scenario/input-resource/{resource_id}",
    tags=["Scenario"],
...
def create_an_scenario(
    scenario: ScenarioSaveDTO, _=Depends(AuthorizationService.check_user_access_token)
) -> ScenarioDTO:
    """
    Create a scenario.
    """

    return ScenarioService.create_scenario_from_dto(scenario).to_dto()
...
@core_app.put("/scenario/{id_}/title", tags=["Scenario"], summary="Update the title of a scenario")
def update_title(
...
@core_app.put(
    "/scenario/{id_}/folder", tags=["Scenario"], summary="Update the folder of a scenario"
)
...
@core_app.put(
    "/scenario/{id_}/description", tags=["Scenario"], summary="Update a scenario's description"
)
def update_scenario_description(
    id_: str, description: RichTextDTO, _=Depends(AuthorizationService.check_user_access_token)
) -> ScenarioDTO:
    """
...
@core_app.get(
    "/scenario/{id_}/reset/check-impact",
    tags=["Scenario"],
    summary="Check impact for scenario reset",
)
def check_impact_for_scenario_reset(
    id_: str, _=Depends(AuthorizationService.check_user_access_token)
) -> ImpactResultDTO:
...
def delete_scenario(id_: str, _=Depends(AuthorizationService.check_user_access_token)) -> None:
...
def archive(id_: str, _=Depends(AuthorizationService.check_user_access_token)) -> ScenarioDTO:
...
def unarchive(id_: str, _=Depends(AuthorizationService.check_user_access_token)) -> ScenarioDTO:
...
@core_app.delete(
    "/scenario/{id_}/intermediate-resources",
    tags=["Scenario"],
    summary="Delete all intermediate resources of a scenario",
)
def delete_intermediate_resources(
    id_: str, _=Depends(AuthorizationService.check_user_access_token)
) -> None:
...
@core_app.post(
    "/scenario/import-from-lab", tags=["Share"], summary="Import a scenario from another lab"
)
def import_from_lab(
...
@core_app.get(
    "/scenario/import-from-lab/config-specs",
    tags=["Share"],
...
@core_app.post(
    "/scenario/{id_}/export-to-lab", tags=["Share"], summary="Export a scenario to another lab"
)
def export_to_lab(
...
@core_app.get(
    "/scenario/export-to-lab/config-specs",
    tags=["Share"],
