...
async def update_a_view_model(uri: str, \
    ...
    from gws.service.view_service import ViewService
    
    vm = ViewService.update_view_model( 
        uri = uri, 
        data = data    
    )
...
async def get_a_view_model(uri: str, \
    ...
    vm = ViewService.fetch_view_model(uri=uri)
    try:
        return vm.to_json()
    except Exception as err:
        raise HTTPInternalServerError(detail=f"Cannot render view. The rendering function '{data.render}' may not be valid", debug_error=err)
...
async def get_the_list_of_view_models(model_type: Optional[str] = None, \
                            page: Optional[int] = 1, \
