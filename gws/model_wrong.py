...
class Experiment(Viewable):
    ...
    :type parent: `gws.model.Experiment`
    ...
    def from_flow(cls, flow: dict) -> 'Experiment':
        """ 
        Create a new instance from a existing job flow
    ...
    def is_finished(self):
        ...
        :return: True if the experiment is finished, i.e. all its job are finished. False otherwise
        :rtype: `bool`
        """
    ...
    def is_running(self) -> bool:
        ...
        :return: True if the experiment is running, i.e. all its job are running. False otherwise
        :rtype: `bool`
        """
    ...
    def job(self) -> 'Job':
        """ 
        Return the job of the experiment, i.e. the job associated with the protocol of the experiment.
    ...
    def run_cli(self, *, user=None, is_test=False):
        ...
        #TICK_INTERVAL_SECONDS = 5   # 5 min
        #def _monitor_experiment():  
        #    t = threading.Timer(TICK_INTERVAL_SECONDS, _monitor_experiment, daemon=True)
        #    t.start()
        #
        #_monitor_experiment()
...
class Job(Viewable):
    ...
    @property
    def is_running(self):
        return self.progress_bar.is_running
    

    @property
    def is_finished(self):
        return self.progress_bar.is_finished
    ...
    def process(self):
        ...
        if self.process_uri:
            ...
            self._process = process_t.get(process_t.uri == self.process_uri)
    ...
    def __track_input_flow(self):
        ...
        for k in _input.ports:
            ...
            if res is None:
                ...
                if not res.is_saved():
                    raise Error("gws.model.Process", "__track_input_flows", f"Cannot track input '{k}' uri. Please save the input resource before.")
