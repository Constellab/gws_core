...
class TestResourceModel(BaseTestCase):
    def test_search_by_name(self):
        ...
        resource_model_2 = ResourceModel.save_from_resource(
            resource_2, origin=ResourceOrigin.UPLOADED
        )
    ...
    def test_search(self):
        ...
        search_dict.set_filters_criteria(
            [
                SearchFilterCriteria(
                    key="origin", operator=SearchOperator.EQ, value=ResourceOrigin.GENERATED.value
                )
            ]
        )
        ...
        search_dict.set_filters_criteria(
            [
                SearchFilterCriteria(
                    key="scenario", operator=SearchOperator.EQ, value=scenario._scenario.id
                )
            ]
        )
        self._search(search_dict, 1)
        ...
        search_dict.set_filters_criteria(
            [
                SearchFilterCriteria(
                    key="task_model", operator=SearchOperator.EQ, value=task._process_model.id
                )
            ]
        )
        self._search(search_dict, 1)
    ...
    def test_update_type(self):
        ...
        ResourceService.update_resource_type(resource_model.id, SubFile.get_typing_name())

        resource_model = ResourceModel.get_by_id_and_check(resource_model.id)
