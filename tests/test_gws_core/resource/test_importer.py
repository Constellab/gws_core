...
class TestImporter(BaseTestCase):
    def test_get_import_specs(self):
        ...
        importers = TypingService.search_importers(
            File.get_typing_name(), "anotherweirdformatfile", SearchParams(), 0, 1000
        )
        ...
        search_params = SearchParams(
            filtersCriteria=[
                SearchFilterCriteria(
                    key="importer_ignore_extension", value=True, operator=SearchOperator.EQ
                )
            ]
        )
        importers = TypingService.search_importers(
            File.get_typing_name(), "anotherweirdformatfile", search_params, 0, 1000
        )
    ...
    def test_importer(self):
        ...
        result: ResourceModel = ConverterService.call_importer(
            resource_model.id, TextImporter.get_typing_name(), {}
        )
