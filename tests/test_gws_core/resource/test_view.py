...
class ResourceViewTest(Resource):
    @view(
        view_type=TextView,
        human_name="View for test",
        short_description="Description for test",
        default_view=True,
    )
...
class ResourceViewTestSub(ResourceViewTest):
    @view(
        view_type=TextView,
        human_name="Sub View for test",
        short_description="Description for sub test",
        specs=ConfigSpecs(
            {"test_str_param": StrParam(default_value="Hello"), "test_any_param": StrParam("Nice")}
        ),
        default_view=True,
    )
    def sub_view_test(self, params: ConfigParams) -> TextView:
        text_view = TextView(
            params.get_value("test_str_param") + params.get_value("test_any_param")
        )
        text_view.set_title("Sub view title")
        return text_view
...
class TestView(BaseTestCase):
    def test_view_def(self):
        ...
        sub_view_test: ResourceViewMetaData = [
            x for x in views if x.method_name == "sub_view_test"
        ][0]
    ...
    def test_viewer(self):
        ...
        view_config = {
            "view_method_name": "a_view_test",
            "config_values": {"page": 1, "page_size": 5000},
        }
        viewer = i_protocol.add_process(
            Viewer,
            "viewer",
