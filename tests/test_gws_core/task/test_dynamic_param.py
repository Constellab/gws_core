...
class TestDynamicParam(BaseTestCase):
    ...
    def test(self):
        ...
        ProtocolService.add_dynamic_param_spec_of_process(
            proto.id, process_model.instance_name, 'params', 'a', IntParam(default_value=2, optional=True).to_dto()
        )
        ...
        self.assertEqual(test_process_model.config.get_spec('params').specs['a'].to_dto()
                         .to_json_dict(), IntParam(default_value=2, optional=True).to_dto().to_json_dict())
        ...
        self.assertIsNotNone(test_process_model.config.get_value('params'))
        self.assertEqual(test_process_model.config.get_spec('params') .specs['a'].to_dto(
        ).to_json_dict(), IntParam(default_value=3, optional=True).to_dto().to_json_dict())
        ...
        ProtocolService.rename_and_update_dynamic_param_spec_of_process(
            proto.id, process_model.instance_name, 'params', 'a', 'b', IntParam(default_value=4, optional=True).to_dto()
        )
        ...
        self.assertIsNotNone(test_process_model.config.get_value('params'))
        self.assertEqual(test_process_model.config.get_spec('params') .specs['b'].to_dto(
        ).to_json_dict(), IntParam(default_value=4, optional=True).to_dto().to_json_dict())
