...
class Jump(Task):
    input_specs = InputSpecs(
        {"jump_person_in_1": InputSpec(Person), "jump_person_in_2": InputSpec(Person)}
    )
    output_specs = OutputSpecs(
        {
            "jump_person_out": OutputSpec(Person),
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        return {
            "jump_person_out": inputs["jump_person_in_1"],
            "jump_person_out_any": inputs["jump_person_in_2"],
        }
...
class Multi(Task):
    input_specs = InputSpecs(
        {"resource_1": InputSpec((Car, Person)), "resource_2": InputSpec([Car, Person])}
    )
    output_specs = OutputSpecs(
        {"resource_1": OutputSpec((Car, Person)), "resource_2": OutputSpec([Car, Person])}
    )
    config_specs = ConfigSpecs({})
...
class OptionalTask(Task):
    input_specs = InputSpecs(
        {
            "first": InputSpec([Person], optional=True),
...
class TestPersonProtocol(Protocol):
    def configure_protocol(self) -> None:
        ...
        self.add_connectors([(create >> "create_person_out", log << "person")])
...
class Skippable(Task):
    input_specs: InputSpecs = InputSpecs(
        {
            "resource_1": InputSpec(Resource, optional=True),
...
class TestIO(BaseTestCase):
    def test_connect(self):
        ...
        with self.assertRaises(ImcompatiblePortsException):
            Connector(process_2, process_3, "move_person_out", "move_drive_in")

        self.assertEqual(
            port_connect.to_dto().to_json_dict(),
            {
    ...
    def test_multi(self):
        ...
        Connector(create, multi, "create_person_out", "resource_1")
        Connector(create, multi, "create_person_out", "resource_2")

        # Test that you can plug multi to moves
        Connector(multi, jump, "resource_1", "jump_person_in_1")
        Connector(multi, jump, "resource_2", "jump_person_in_2")
    ...
    def test_sub_class_output(self):
        ...
        Connector(jump, fly, "jump_person_out_any", "superman")
