...
class TestEnvShellProxy(TestCase):
    def test_pip_shell_proxy(self):
        pip_env_file = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "penv", "env_jwt_pip.txt"
        )
        self._test_env_shell_proxy(PipShellProxy, "MyPipTestEnvironment", pip_env_file)

    def test_conda_shell_proxy(self):
        conda_env_file = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "penv", "env_jwt_conda.yml"
        )
    ...
    def _test_env_shell_proxy(
        ...
        try:
            ...
            creation_info: VEnvCreationInfo = shell_proxy_type.get_creation_info(
                shell_proxy.get_env_dir_path()
            )
            ...
            basic_info: VEnvBasicInfoDTO = [
                x for x in status.envs if x.creation_info.name == env_name
            ][0]
