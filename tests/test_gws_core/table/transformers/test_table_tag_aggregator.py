...
class TestTableTagAggregator(TestCase):
    def test_table_tag_aggregator_helper(self):
        ...
        # Mean on row
        grouped_table = TableTagAggregatorHelper.aggregate_by_row_tags(
            table, keys=["gender"], func="mean"
        )
        ...
        # Sum on column
        grouped_table = TableTagAggregatorHelper.aggregate_by_column_tags(
            table, keys=["group"], func="sum"
        )
