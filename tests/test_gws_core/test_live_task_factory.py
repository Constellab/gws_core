...
class TestLiveTaskFactory(BaseTestCase):

    def test_generate_live_task_file_from_live_task_id(self):
        ...
        live_task_dict = LiveTaskFactory().generate_live_task_file_from_live_task_id(streamlit_process.id)

        self.assertEqual(live_task_dict['code'], community_live_task_version.code)
        self.assertEqual(live_task_dict['config_specs'], community_live_task_version.config_specs)
        self.assertEqual(live_task_dict['params'], community_live_task_version.params)
        self.assertIsNotNone(live_task_dict['bricks'])
        self.assertEqual(live_task_dict['task_type'] == 'STREAMLIT', True)
