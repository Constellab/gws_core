...
class TestTagPropagation(BaseTestCase):
    def test_tag_propagation_exp(self):
        ...
        tag_b = Tag(
            "tag_b", "value_b", is_propagable=True, origins=TagOrigins(TagOriginType.USER, user_id)
        )
        tag_exp = Tag(
            "tag_exp",
            "value_exp",
        ...
        first_robot_model = ResourceModel.save_from_resource(
            first_robot, origin=ResourceOrigin.UPLOADED
        )
        ...
        robot_food_model = ResourceModel.save_from_resource(
            robot_food, origin=ResourceOrigin.UPLOADED
        )
        ...
        self._check_propagated_input_tags(
            first_output, tag_a, tag_b, tag_r_not_propagable, tag_robot._process_model.id
        )
        ...
        self.assertTrue(
            output_tag_exp.origins.has_origin(
                TagOriginType.SCENARIO_PROPAGATED, i_scenario.get_model().id
            )
        )

        # Check task tags
        self._check_task_tags(
            first_output,
            task_tag_propagable,
            task_tag_not_propagable,
            tag_robot._process_model.id,
        )
    ...
    def _check_propagated_input_tags(
        self,
        output_resource: Robot,
        tag_a: Tag,
        tag_b: Tag,
        tag_r_not_propagable: Tag,
        task_model_id: str,
    ) -> None:
        """Check that the propagable input tags were propagated to the task output.

        :param output_resource: the resource generated by the task
        :param tag_a: a propagable tag set on the input resource
        :param tag_b: another propagable tag set on the input resource
        :param tag_r_not_propagable: a non propagable tag set on the input resource
        :param task_model_id: the id of the task model that generated the output
        """
        self.assertTrue(output_resource.tags.has_tag(tag_a))
        self.assertTrue(output_resource.tags.has_tag(tag_b))
        self.assertFalse(output_resource.tags.has_tag(tag_r_not_propagable))
        ...
        self.assertTrue(
            output_tag_a.origins.has_origin(TagOriginType.TASK_PROPAGATED, task_model_id)
        )

    def _check_task_tags(
        self,
        output_resource: Robot,
        task_tag_propagable: Tag,
        task_tag_not_propagable: Tag,
        task_model_id: str,
    ) -> None:
        """Check the tags added by the task itself on its output resource.

        :param output_resource: the resource generated by the task
        :param task_tag_propagable: the propagable tag added by the task
        :param task_tag_not_propagable: the non propagable tag added by the task
        :param task_model_id: the id of the task model that generated the output
        """
        ...
        self.assertTrue(
            ouput_tag_task_propagable.origins.has_origin(TagOriginType.TASK, task_model_id)
        )
        ...
        self.assertTrue(
            ouput_tag_task_not_propagable.origins.has_origin(TagOriginType.TASK, task_model_id)
        )
    ...
    def test_tag_propagation_view_note(self):
        ...
        self.assertTrue(
            tag.origins.has_origin(TagOriginType.RESOURCE_PROPAGATED, resource_model.id)
        )
    ...
    def test_tag_propagation_after(self):
        ...
        self._check_propagation_exp_1(
            exp_1.id,
            exp_1_output.id,
            exp_2_output.id,
            exp_2.id,
            exp_2_note.id,
            exp_2_output_view.id,
            1,
            new_tag,
            True,
        )
        ...
        TagService.delete_tag_from_entity(
            TagEntityType.SCENARIO, exp_1.id, new_tag.key, new_tag.value
        )
        ...
        TagService.delete_tag_from_entity(
            TagEntityType.SCENARIO, exp_2.id, new_tag_2.key, new_tag_2.value
        )
