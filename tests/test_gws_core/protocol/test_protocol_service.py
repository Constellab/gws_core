...
class TestProtocolService(BaseTestCase):
    def test_add_process(self):
        ...
        self.assertEqual(
            protocol_model.get_process(process_model.instance_name).get_process_type(), RobotMove
        )
        ...
        ProtocolService.rename_process(
            protocol_model.id, process_model.instance_name, "New super name"
        )
        process_model = process_model.refresh()
    ...
    def test_add_viewer(self):
        ...
        self.assertEqual(
            viewer_model.config.get_value(Viewer.resource_config_name), Robot.get_typing_name()
        )
    ...
    def test_reset_process(self):
        ...
        EntityNavigatorService.reset_process_of_protocol_id(sub_protocol.get_model().id, "p2")
        ...
        self.assertTrue(sub_protocol.get_process("p_wait").get_model().is_draft)
        ...
        scenario2.get_protocol().add_resource("source", main_resource.id, robot_mode << "robot")
    ...
    def test_duplicate_process_to_protocol_id(self):
        ...
        self.assertEqual(
            duplicated_process.to_config_dto().inputs, process_model.to_config_dto().inputs
        )
        self.assertEqual(
    ...
    def test_add_streamlite_community_agent_version_to_protocol(self):
        ...
        community_agent_version: CommunityAgentVersionDTO = CommunityAgentVersionDTO.from_json(
            {
                ...
                "config_specs": {},
                "agent": {
                    "id": "d4816577-94b6-4cc7-a363-52127f4e4525",
        ...
        duplicated_streamlit_process = protocol_model.get_process(
            streamlit_base_process_name + "_1"
        )
        ...
        community_agent_version: CommunityAgentVersionDTO = CommunityAgentVersionDTO.from_json(
            {
                ...
                "params": {"specs": {}, "values": {}},
                "input_specs": {"specs": {"source": first_input, "table": second_input}},
                "output_specs": {"specs": {"target": first_output, "table": second_output}},
                "config_specs": {},
                "agent": {
                    "id": "d4816577-94b6-4cc7-a363-52127f4e4525",
                    "title": "Test PyAgent",
