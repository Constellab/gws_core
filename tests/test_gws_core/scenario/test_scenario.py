...
class TestScenario(BaseTestCase):
    def test_create_empty(self):
        ...
        ScenarioService.update_scenario_description(scenario.id, rich_text.to_dto())
    ...
    def test_run_through_cli(self):
        ...
        result = scenario_waiter.wait_until_finished(
            refresh_interval=3, refresh_interval_max_count=10
        )
    ...
    def test_reset(self):
        ...
        self._check_process_reset(ScenarioService.get_by_id_and_check(scenario.id).protocol_model)
    ...
    def test_protocol_copy(self):
        ...
        self.assertEqual(ProtocolModel.select().count(), count_protocol - (added_protocol * 2))
        self.assertEqual(TaskModel.select().count(), count_task - (added_task * 2))
        # resources are not copied
        self.assertEqual(ResourceModel.select().count(), count_resource - added_resource)

        ScenarioRunService.run_scenario(scenario_copy)
        self.assertEqual(ResourceModel.select().count(), count_resource - (added_resource * 2))
