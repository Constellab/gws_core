...
class TestTableAggregatorHelper(BaseTestCase):
    async def test_table_aggregator_helper(self):
        ...
        df = DataframeAggregatorHelper.aggregate(data=table.get_data(), direction="vertical", func="sum")
        exptect_df = DataFrame(data=[110, 170], index=["Age", "Weight"]).T
        self.assertTrue(df.equals(exptect_df))

        df = DataframeAggregatorHelper.aggregate(data=table.get_data(), direction="vertical", func="mean")
        exptect_df = DataFrame(data=[27.5, 42.5], index=["Age", "Weight"]).T
        self.assertTrue(df.equals(exptect_df))

        df = DataframeAggregatorHelper.aggregate(data=table.get_data(), direction="horizontal", func="median")
